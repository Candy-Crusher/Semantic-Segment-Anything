...
def parse_args():
    ...
    parser.add_argument('--save_sem_map', default=False, action='store_true', help='whether to save sem map predicted by semantic branch')
    ...
    parser.add_argument('--dataset', type=str, default='ade20k', choices=['ade20k', 'cityscapes', 'foggy_driving', 'dsec'], help='specify the set of class names')
    ...
    parser.add_argument('--points_per_side', type=int, default=64, help='The number of points sampled along one side of the image. The total number of points is point_per_side**2')
    parser.add_argument('--crop_n_layers', type=int, default=1, help='如果>0 蒙版预测将在图像的裁剪上再次运行。设置运行的层数 其中每层有2**i_layer的图像裁剪数')
...
def main(rank, args):
    ...
    mask_branch_model = SamAutomaticMaskGenerator(
        ...
        # points_per_side=128 if (args.dataset == 'foggy_driving' or args.dataset == 'dsec') else 64,
    ...
    for i, file_name in enumerate(sorted(local_filenames)):
        ...
        if args.dataset == 'ade20k':
            ...
            raise NotImplementedError()
        with torch.no_grad():
            semantic_segment_anything_inference(file_name, args.out_dir, rank, img=img, 
                                   save_img=args.save_img,
                                   save_sem_map=args.save_sem_map,
