...
def parse_args():
    ...
    parser.add_argument('--scene', help='day or night')
...
if args.dataset == 'dsec':
        if args.scene == 'day':
            prefixs = ['zurich_city_13_a', 'zurich_city_14_c', 'zurich_city_15_a']
        else:
...
for split in tqdm(prefixs, desc="Split loop"):
    ...
    for i, fn_ in enumerate(tqdm(filenames, desc="File loop")):
        ...
        seg_pred_path = os.path.join(res_path_split, fn_.replace('_semantic.json','_labelTrainIds.png'))
        seg_pred_img_bytes = file_client.get(seg_pred_path)
        seg_pred = mmcv.imfrombytes(
            seg_pred_img_bytes, flag='unchanged',
